...
class AmmoCollisionBackend {
    ...
    collide(query) {
        ...
        for (const offset of basis) {
            const rayHit = this._castRay(query.start.clone().add(offset), query.end.clone().add(offset), movement, offset);
            if (!rayHit)
                continue;
    ...
    applyImpulse(hit, impulse) {
        ...
        if (!(data === null || data === void 0 ? void 0 : data.body))
            return;
        if (typeof data.body.getCenterOfMassPosition !== 'function'
            || typeof data.body.applyImpulse !== 'function') {
            return;
        }
        const ammoImpulse = new this.Ammo.btVector3(impulse.x, impulse.y, impulse.z);
        const worldPoint = new this.Ammo.btVector3(hit.point.x, hit.point.y, hit.point.z);
        const center = data.body.getCenterOfMassPosition();
        const relativePosition = new this.Ammo.btVector3(worldPoint.x() - center.x(), worldPoint.y() - center.y(), worldPoint.z() - center.z());
        data.body.activate(true);
        data.body.applyImpulse(ammoImpulse, relativePosition);
        this._destroy(ammoImpulse, worldPoint, relativePosition);
    }
    _castRay(start, end, movement, offset) {
        const fromPosition = new this.Ammo.btVector3(start.x, start.y, start.z);
        const toPosition = new this.Ammo.btVector3(end.x, end.y, end.z);
        const callback = new this.Ammo.ClosestRayResultCallback(fromPosition, toPosition);
    ...
    _getSweepBasis(direction, radius) {
        if (radius <= 0)
    ...
    _getRigidBody(collisionObject) {
        var _a, _b;
        if (this.Ammo.castObject) {
            return this.Ammo.castObject(collisionObject, this.Ammo.btRigidBody);
        }
        return (_b = (_a = this.Ammo.btRigidBody.prototype).upcast) === null || _b === void 0 ? void 0 : _b.call(_a, collisionObject);
    }
    _destroy(...objects) {
